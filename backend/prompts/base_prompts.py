...
class InstructionPrompt(RolePrompt):
    """
    Represents a reusable system-level instruction prompt for the LLM.

    Parameters
    ----------
    prompt : str
        The instruction text to inject as a system message for guiding LLM behavior.
    """

    def __init__(self, prompt: str):
        super().__init__(prompt, "system")

class QueryPrompt(RolePrompt):
    """
    Formats a user query and optional context into a prompt for the LLM.

    Parameters
    ----------
    prompt : str
        The base template string with placeholders for user question and context.
    """
